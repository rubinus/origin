...
func (repo *Weather) Insert(ctx context.Context, wea *Weather) (result string, err error) {
  ...
  //取db连接
  coll := zgo.Mongo.GetCollection(DBLabel, CollLabel, MongoLabel)
  result, err = zgo.Mongo.Insert(ctx, coll, wea)
  if err != nil {
    zgo.Log.Error(err)
    return
  }
...
func (repo *Weather) List(ctx context.Context, city string) (weathers []*Weather, err error) {
  ...
  //取db连接
  coll := zgo.Mongo.GetCollection(DBLabel, CollLabel, MongoLabel)
  results, err := zgo.Mongo.Find(ctx, coll, args)
  if err != nil {
    return
  }
