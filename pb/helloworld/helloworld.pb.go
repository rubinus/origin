...
func (m *HelloRequest_Request) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if len(m.Ins) > 0 {
		for iNdEx := len(m.Ins) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.Ins[iNdEx])
			copy(dAtA[i:], m.Ins[iNdEx])
			i = encodeVarintHelloworld(dAtA, i, uint64(len(m.Ins[iNdEx])))
			i--
			dAtA[i] = 0x1a
		}
	}
...
func (m *HelloResponse) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.MyMap) > 0 {
		for k := range m.MyMap {
			...
			i--
			dAtA[i] = 0xa
			i = encodeVarintHelloworld(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x2a
		}
	}
	...
	if len(m.Ans) > 0 {
		for iNdEx := len(m.Ans) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.Ans[iNdEx])
			copy(dAtA[i:], m.Ans[iNdEx])
			i = encodeVarintHelloworld(dAtA, i, uint64(len(m.Ans[iNdEx])))
			i--
			dAtA[i] = 0x1a
		}
	}
