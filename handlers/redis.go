...
func RedisPut(ctx iris.Context) {
	name := ctx.URLParam("name")
	...
	result, err := zgo.Redis.Set(cotx, key, s)
	if err != nil {
		zgo.Log.Error(err)
		return
	}
	...
	select {
	case <-cotx.Done():
		errStr = "call redis put string timeout"
		zgo.Log.Error(errStr) //通过zgo.Log统计日志
		//ctx.JSONP(iris.Map{"status": 201, "msg": errStr}) //返回jsonp格式
		zgo.Http.JsonpErr(ctx, errStr)
	default:
...
func RedisHashGet(ctx iris.Context) {
	name := ctx.URLParam("name")
	...
	//get String key return a map
	val, err := zgo.Redis.Hget(cotx, key, "test")
...
func RedisHashPut(ctx iris.Context) {
	name := ctx.URLParam("name")
	...
	//set stander json to redis String
	result, err := zgo.Redis.Hset(cotx, key, "test", s)
	if err != nil {
		zgo.Log.Error(err)
		return
	}
